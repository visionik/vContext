# vAgenda: Atomic TRON Classes Proposal
...
This document proposes a set of atomic TRON classes that capture common patterns across vAgenda's core data structures. By defining these atomic classes, we can:
...
**All vAgenda entities MUST:**
- Be **Titled** (have `title` and optional `description`)

**All vAgenda lifecycle entities (TodoList, TodoItem, Plan, Phase, Playbook, PlaybookEntry) MUST:**
...
**All vAgenda entities MAY (via extensions):**
...
These atomics MUST be present in all vAgenda entities.
...
**Used by**: ALL vAgenda entities (TodoList, TodoItem, Plan, Phase, Playbook, PlaybookEntry, ProblemModel)

**Rationale**: Every vAgenda entity must be understandable to humans and agents. A title is the minimum viable description. The description field provides an escape hatch for additional context without requiring new fields.
...
**Used by**: ALL vAgenda entities except containers (TodoList, Playbook) and pure structures (ProblemModel)
...
**Rationale**: Tags enable flexible categorization without rigid taxonomies. Essential for filtering, searching, and grouping across vAgenda documents. Common use cases:
...
2. **Validate with Examples**: Ensure all current vAgenda examples can be expressed with Item base
